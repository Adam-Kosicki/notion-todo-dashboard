...
This public source tree contains no user records, provider tokens, database contents, deployment credentials, or live Notion data-source IDs. Configure private values only in your hosting provider's secret and environment-variable settings—never in committed files.
...
The D1 database binding is named `DB`. Each user's rows are scoped by the authenticated user ID supplied by the hosting layer. Do not commit `.env` files, database exports, encrypted seed files, or provider credentials.
